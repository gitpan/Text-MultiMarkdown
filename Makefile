# This Makefile is for the Text::MultiMarkdown extension to perl.
#
...
#     ABSTRACT => q[Convert MultiMarkdown syntax to (X)HTML]
#     AUTHOR => q[Darren M. Kulp <kulp@cpan.org>]
#     DIR => []
#     DISTNAME => q[Text-MultiMarkdown]
#     NAME => q[Text::MultiMarkdown]
#     NO_META => q[1]
#     PL_FILES => {  }
#     PREREQ_PM => { Scalar::Util=>q[0], Test::Exception=>q[0], Encode=>q[0], Test::More=>q[0.42], List::Util=>q[0], MIME::Base64=>q[0], Text::Balanced=>q[0], Digest::MD5=>q[0], List::MoreUtils=>q[0], File::Slurp=>q[0] }
#     VERSION => q[1.0.8]
#     dist => { PREOP=>q[$(PERL) -I. -MModule::Install::Admin -e "dist_preop(q($(DISTVNAME)))"] }
#     test => { TESTS=>q[t/01use.t t/02pod.t t/03podcoverage.t t/03podspelling.t t/04markdown.t t/05options.t t/06wiki.t t/07fulldoc.t t/08exporter.t t/09base_url.t t/10use_metadata.t t/11docment_format.t t/12empty_element_suffix.t t/13multimarkdown-nometadata-firstpara.t t/14unicode.t t/15inlinehtmldoenoturnoffmarkdown.t t/20fulldocs-multimarkdown.t t/21fulldocs-markdown.t t/22fulldocs-multimarkdown-todo.t t/23fulldocs-markdown-todo.t t/24fulldocs-pythonmarkdown2-tm.t t/26fulldocs-php-markdown.t t/27fulldocs-php-markdown-fails.t t/28fulldocs-php-markdown-extra.t t/30fulldocs-maruku-unittest.t] }
...
MAKE = make
FULLEXT = Text/MultiMarkdown
BASEEXT = MultiMarkdown
PARENT_NAME = Text
DLBASE = $(BASEEXT)
VERSION_FROM = 
OBJECT = 
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Text/MultiMarkdown.pm
...
TO_INST_PM = lib/Text/MultiMarkdown.pm

PM_TO_BLIB = lib/Text/MultiMarkdown.pm \
...
DISTNAME = Text-MultiMarkdown
DISTVNAME = Text-MultiMarkdown-1.0.8
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/Text/MultiMarkdown.pm $(INST_MAN3DIR)/Text::MultiMarkdown.$(MAN3EXT) 
...
TEST_FILE = test.pl
TEST_FILES = t/01use.t t/02pod.t t/03podcoverage.t t/03podspelling.t t/04markdown.t t/05options.t t/06wiki.t t/07fulldoc.t t/08exporter.t t/09base_url.t t/10use_metadata.t t/11docment_format.t t/12empty_element_suffix.t t/13multimarkdown-nometadata-firstpara.t t/14unicode.t t/15inlinehtmldoenoturnoffmarkdown.t t/20fulldocs-multimarkdown.t t/21fulldocs-markdown.t t/22fulldocs-multimarkdown-todo.t t/23fulldocs-markdown-todo.t t/24fulldocs-pythonmarkdown2-tm.t t/26fulldocs-php-markdown.t t/27fulldocs-php-markdown-fails.t t/28fulldocs-php-markdown-extra.t t/30fulldocs-maruku-unittest.t
TESTDB_SW = -d
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="1,0,8,0">' > $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <TITLE>$(DISTNAME)</TITLE>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>Convert MultiMarkdown syntax to (X)HTML</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Darren M. Kulp &lt;kulp@cpan.org&gt;</AUTHOR>' >> $(DISTNAME).ppd
	...
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Text-Balanced" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' -- \
	  lib/Text/MultiMarkdown.pm blib/lib/Text/MultiMarkdown.pm 
